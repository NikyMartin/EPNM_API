...
"""
    ...
    This script uses REST to extract following details from IP Interfaces
      - Interface Name
      - IP Address
      - Admin State
      - Operational State
...
# Execution
    ...
    try:
        interface_list = parsed["queryResponse"]["entity"][0]["inventoryDetailsDTO"]["ipInterfaces"]["ipInterface"]
    except:
        print("ERROR: Could not locate ipInterface")
        exit(1)
    ...
    for interface in interface_list:
        try:
            ...
            interface_name = ""
        try:
            interface_address = interface["ipAddress"]["address"]
        except:
            interface_address = ""
        try:
        ...
        entry = (interface_name, interface_address, interface_adminStatus, interface_operationalStatus)
        interface_list_output.append(entry)

    print(tabulate(sorted(interface_list_output), headers=(['Name', 'IP Address', 'Admin Status', 'Op Status'])))
